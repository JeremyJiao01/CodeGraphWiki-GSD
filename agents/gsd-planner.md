...
For brownfield codebases, **always search before planning**. The answer to most requirements is "call existing function X in order Y" — not "write new code".
...
Load codebase context. Check which format is available, preferring the CodeGraphWiki format.

---

### Path A: CodeGraphWiki format (preferred)

```bash
ls .planning/codebase/INDEX.md 2>/dev/null
```

**If INDEX.md exists**, use CodeGraphWiki:

**1. Read the index to see available wiki pages and graph stats:**
```bash
cat .planning/codebase/INDEX.md
```

**2. Read relevant wiki pages** by matching their titles to the phase topic.

| Phase Keywords | Prioritize wiki pages about... |
|----------------|-------------------------------|
| UI, frontend, components | conventions, structure, frontend patterns |
| API, backend, endpoints | architecture, API reference, backend patterns |
| database, schema, models | data models, schema, database |
| testing, tests | testing patterns, quality, coverage |
| integration, external API | integrations, external services, dependencies |
| refactor, cleanup | architecture, concerns, tech debt |
| setup, config | stack, infrastructure, configuration |
...
### Path B: Legacy static files (fallback)

```bash
ls .planning/codebase/*.md 2>/dev/null
```

**If static files exist (no INDEX.md)**, load relevant docs by phase type:
...
Proceed without codebase context. Consider running `/gsd:map-codebase` first for brownfield projects.
</step>
