...
|----------|------|---------|-------------|
| **0th** | **code-graph-builder** | **Existing codebase: what's already implemented, how it's structured, call graphs** | **HIGHEST** |
| 1st | Context7 | Library APIs, features, configuration, versions | HIGH |
| 2nd | WebFetch | Official docs/READMEs not in Context7, changelogs | HIGH-MEDIUM |
| 3rd | WebSearch | Ecosystem discovery, community patterns, pitfalls | Needs verification |

## CodeGraphWiki (Priority 0) — Check Existing Code First
...
Invoke Skill `/code-graph:repo-info` — if the project is not found, skip CodeGraphWiki and proceed to Context7/WebSearch.

**Key Skills:**
| Skill | When to Use |
|-------|-------------|
| `/code-graph:api-find "..."` | **Primary: Find APIs matching requirements** — returns signatures + call graphs |
| `/code-graph:code-search "..."` | Find existing implementations by topic ("authentication middleware") |
| `/code-graph:graph-query "..."` | Understand call relationships ("what calls X?", "what does module Y depend on?") |
| `/code-graph:wiki-read <page_id>` | Read architectural wiki pages about the codebase |
| `/code-graph:wiki-list` | Discover available wiki pages |

**Research flow with CodeGraphWiki:**
1. `/code-graph:api-find "<requirement or feature>"` — **start here**: find all APIs relevant to this phase's requirements. Run once per major requirement group.
2. `/code-graph:code-search "<phase topic>"` — supplement with semantic search if api-find misses something
3. `/code-graph:graph-query "how is X connected to Y?"` — understand relationships between discovered APIs
